...
const Game = ({ title, image, className }) => {
  return (
    <div
      className={`flex flex-col items-center rounded-lg transition-transform transform hover:scale-95 ${className}`}
    >
      <div
        className="cursor-pointer w-40 h-40 bg-gray-200 text-center overflow-hidden rounded-2xl"
        onClick={() =>
          (window.location.href = `/games/${title
            .replace(/\s+/g, "-")
            .toLowerCase()}`)
        }
      >
...
const GameSection = () => {
  ...
  return (
    <div className="text-left w-full mt-5 mb-3">
      <h3 className="text-2xl font-bold pl-5 mb-4 text-white">I nostri giochi</h3>
